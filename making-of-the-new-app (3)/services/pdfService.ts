 ...
 * Uses ultra-high DPI (scale) to ensure text remains sharp even when re-encoded.
 */
export async function compressPdf(file: File, level: CompressionLevel = 'Standard'): Promise<Blob> {
  ...
  // High-DPI scaling factors to prioritize text legibility
  const settings = {
    // Standard: Ultra-High DPI (3.5x) - Text quality is visually indistinguishable from original.
    'Standard': { scale: 3.5, quality: 0.92 }, 
    // High: High DPI (2.5x) - Excellent clarity for documents with many images.
    'High': { scale: 2.5, quality: 0.70 }, 
    // Maximum: Moderate DPI (1.8x) - Focuses on size reduction while keeping text legible.
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    if (context) {
        // Enforce high-quality interpolation for text rendering
    ...
    // We use JPEG with high quality to minimize compression artifacts around characters
